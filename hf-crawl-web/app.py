...
# --- Config ---
API_URL = os.environ.get("API_URL", "http://192.168.1.240:8001")
...
if page == "Overview":
    ...
    resp = requests.get(f"{API_URL}/stats", timeout=30).json()
    ...
    sql_resp = requests.post(f"{API_URL}/sql", json={"query": "SELECT pipeline_tag, COUNT(*) as cnt FROM models_list GROUP BY pipeline_tag ORDER BY cnt DESC LIMIT 20"}, timeout=30).json()
    ...
    models = requests.get(f"{API_URL}/models", params=params, timeout=30).json().get("models", [])
    ...
    if model_id:
        resp = requests.get(f"{API_URL}/model/{model_id}", timeout=30)
        if resp.status_code == 200:
            info = resp.json()
            st.json(info)
        else:
    ...
    model_id = st.text_input("Enter model ID", "albert/albert-base-v2")
    if model_id:
        resp = requests.get(f"{API_URL}/cards/{model_id}", timeout=30)
        if resp.status_code == 200:
            card = resp.json()
